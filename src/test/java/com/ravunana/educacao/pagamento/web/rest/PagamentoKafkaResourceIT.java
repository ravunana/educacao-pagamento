...
import com.ravunana.educacao.pagamento.config.KafkaProperties;
...
class PagamentoKafkaResourceIT {
    ...
    void setup() {
        ...
        PagamentoKafkaResource kafkaResource = new PagamentoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/pagamento-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/pagamento-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
